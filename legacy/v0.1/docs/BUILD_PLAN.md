...
- **Hybrid fork**: **Hermes Agent (Python) as the core "brain"** + **OpenClaw channel capabilities** ported on top.
- **Connect ruflo as an MCP server** (native tools) — may need one Claude Code restart.
- **v1 priority = stealth browser.**

Both upstreams are **MIT-licensed** → fork/port permitted; we retain their LICENSE files + add a NOTICE with attribution.

**Disposition of the current v0.1 `edith/` package**: it becomes a **patch set**. Its best modules
— stealth browser design, skill protection lock, memory namespacing/dedup, authorization gate —
get **ported onto the Hermes fork**. Pending code/security review fixes get applied during that
port. The standalone `edith/` package moves to `legacy/` (kept for reference, not shipped).
